...
const botonSiguienteOrganizador = document.getElementById(
  "boton-sig-organizador",
);
...
const inputNombreParticipante = document.getElementById(
  "input-nombre-participante",
);
...
function guardarExcepcionesActuales(elemento) {
  const participantes = obtenerParticipantes();
  const noMarcados = elemento.querySelectorAll(
    ".container-checkbox-registros input:not(:checked)",
  );
...
function inicializarTarjetaResumen(elemento) {
  ...
  document
    .getElementById("btn-proceder-sorteo")
    .addEventListener("click", () => {
      const parejas = realizarIntercambio();

      if (!parejas) {
        alert(
          "No fue posible realizar el intercambio, revisa las exclusiones.",
        );
        return;
      }
